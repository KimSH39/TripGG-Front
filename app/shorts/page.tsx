        ...
        {shorts.map((short, index) => (
          <div
            ...
            className={`absolute inset-0 transition-transform duration-300 ${
              index === currentVideo ? "translate-y-0" : index < currentVideo ? "-translate-y-full" : "translate-y-full"
            }`}
          >
            {/* Video/Image */}
      ...
      <div className="absolute right-2 top-1/2 transform -translate-y-1/2 space-y-2">
        {shorts.map((_, index) => (
          <button
            ...
            className={`w-2 h-8 rounded-full transition-colors ${index === currentVideo ? "bg-white" : "bg-gray-500"}`}
          />
        ))}
      </div>

      <BottomNavigation currentTab="shorts" />
    </div>
  )
}
